...
float32_t testInput_2048[TEST_LENGTH_SAMPLES]; 
  ...
  while(1){
    ...
    if(temp){
      for(int i=0;i<TEST_LENGTH_SAMPLES;i+=2){
        testInput_2048 [i] = (temp->data)[i/2];
      }
      for(int i=1;i<TEST_LENGTH_SAMPLES;i+=2){
        testInput_2048 [i] = 0;
      }
      arm_cfft_f32(&arm_cfft_sR_f32_len1024, testInput_2048, ifftFlag, doBitReverse);
      #define start_x 162
      ...
      arm_cfft_f32(&arm_cfft_sR_f32_len1024, testInput_2048, true, doBitReverse);
      arm_cmplx_mag_f32(testInput_2048, testOutput, fftSize);
